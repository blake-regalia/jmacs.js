...
for(let s_prop of Object.getOwnPropertyNames(String.prototype)) {
	let f_method = String.prototype[s_prop];
	if('function' === typeof f_method && 'constructor' !== s_prop) {
		H_BUILTIN_FUNCTIONS[s_prop] = (...a_args) => {
			return f_method.call(...a_args);
		};
	}
}

for(let s_prop of Object.getOwnPropertyNames(Array.prototype)) {
	let f_method = Array.prototype[s_prop];
	if('function' === typeof f_method && 'constructor' !== s_prop) {
		// don't conflict String methods
		if(!(s_prop in H_BUILTIN_FUNCTIONS)) {
			H_BUILTIN_FUNCTIONS[s_prop] = (...a_args) => {
				return f_method.call(...a_args);
			};
		}
		// coexist with String
			...
			H_BUILTIN_FUNCTIONS[s_prop] = (z_inst, ...a_args) => {
				if(Array.isArray(z_inst)) {
					return f_method.call(z_inst, ...a_args);
				}
				else {
					return String.prototype[s_prop].call(z_inst, ...a_args);
				}
			};
		}
	}
...
const h_program = {
	...
	assignment(h_assign, h_context) {
		...
		h_scope[s_variable] = this.evaluate(h_assign.expression, h_context, true);
		return '';
	},
...
module.exports = (a_sections) => {
	return function(p_cwd, h_global=null) {
		if(!h_global) h_global = Object.assign({}, H_BUILTIN_CONSTANTS);
				...
				switch(h_expr.type) {
					...
					case 'variable': {
						...
						if(s_name in h_context) {
							...
							if(!b_no_print && Array.isArray(z_value)) {
					...
					case 'code': {
						...
						let h_eval = f_parse(p_cwd, h_context);
					...
					case 'binary_expression': {
						let z_lhs = this.evaluate(h_expr.left, h_context, b_no_print);
						let z_rhs = this.evaluate(h_expr.right, h_context, b_no_print);
					...
					case 'unary_expression': {
						let z_expr = this.evaluate(h_expr.expression, h_context, b_no_print);
						switch(h_expr.operator) {
							case '-': return -z_expr
							case '!': return !z_expr;
							default: {
					...
					case 'ternary_condition': {
						if(this.evaluate(h_expr.if, h_context)) {
							return this.evaluate(h_expr.then, h_context, b_no_print);
						}
						else {
							return this.evaluate(h_expr.else, h_context, b_no_print);
						}
					}
					default:
						debugger;
