...
module.exports = {
	...
	BacktickValue: (s_code) => ({
		type: 'code',
		value: s_code.slice(1, -1),
	}),
	...
	Macro: (s_name, a_params, a_body) => {
		if(a_body.length) {
			if('verbatim' === a_body[0].type) {
				let h_section = a_body[0];
				let s_value = h_section.value.replace(/^\s+/, '');
				if('\\' === s_value[0]) s_value = s_value.slice(1);
				h_section.value = s_value;
			}
			let h_section_last = a_body[a_body.length-1];
			if('verbatim' === h_section_last.type) {
				h_section_last.value = h_section_last.value.replace(/\s+$/, '');
			}
		}
